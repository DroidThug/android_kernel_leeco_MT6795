...
ssize_t (*s3320_product_id_show)(struct device *dev, struct device_attribute *attr, char *buf) = test_sysfs_product_id_show;
...
static ssize_t test_sysfs_product_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int count = 0;
	return snprintf(buf, PAGE_SIZE - count, "product_id:%s,The TP Module isn't Check Calibration data\n",f54->rmi4_data->rmi4_mod_info.product_id_string);
}
