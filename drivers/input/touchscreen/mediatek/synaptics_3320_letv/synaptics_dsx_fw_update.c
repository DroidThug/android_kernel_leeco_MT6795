...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		if(!strcmp(fwu->product_id, "X3-04-01"))
			strncpy(fwu->image_name, FW_IMAGE_AUO, MAX_IMAGE_NAME_LEN);
		else if(!strcmp(fwu->product_id, "X3-05-01"))
			strncpy(fwu->image_name, FW_IMAGE_BOE, MAX_IMAGE_NAME_LEN);
		else{
			dev_err(&fwu->rmi4_data->i2c_client->dev,
...
static int synaptics_fw_updater(unsigned char *fw_data)
{
	int retval;
...
{
	synaptics_rmi4_new_function_s(&fwu_module, true);
...
{
	synaptics_rmi4_new_function_s(&fwu_module, false);
