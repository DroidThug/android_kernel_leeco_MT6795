...
static void gf_hw_power_enable(u8 onoff)
{
	...
	if (onoff && enable) {
                ...
                hwPowerOn(MT6331_POWER_LDO_VMCH, VOL_3000, "fingerprint");
		enable = 0;
	}
...
inline static void gf518m_irq_cfg(void)
{
...
inline static int gf518m_pull_miso_high(void)
{
        ...
        gf516m_debug(INFO_LOG, "%s, gf518m_pull_miso_high success\n", __func__);
        ...
        gf516m_debug(INFO_LOG, "%s, gf518m_pull_miso_high set reset pin\n", __func__);

        return EACCES;
}

/*Confure the IRQ pin for GF516M irq if necessary*/
inline static void gf518m_gpio_cfg(void)
{
        ...
        gf516m_debug(INFO_LOG, "%s, gf518m_gpio_cfg success\n", __func__);
        return;
}
...
static void gf_irq_gpio_cfg(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,goodix-fp");
	if (1) {
		gf_dev->irq_num = 3;//irq_of_parse_and_map(node, 0);
