...
static int lsm6ds0_init_flag = -1;
...
/*---------------------------------------------------------------------------*/
#define DEBUG 1
#define LSM6DS0_GYRO_MTK_NEW_SENSOR_ARCH
static DEFINE_MUTEX(lsm6ds0_gysensor_mutex);
/*----------------------------------------------------------------------------*/
...
static int lsm6ds0_local_init(void)
{
	printk("dean add lsm6ds0 local init enter!\n"); 
    ...
    {       
            GYRO_ERR("lsm6ds0 init flag error\n");
            return -1;
    }
	//GYRO_ERR("lsm6ds0_local_init exit \n");
	printk("deans add lsm6ds0_local_init exit!\n");
...
static int lsm6ds0_enable_nodata(int en)
{
    ...
    mutex_lock(&lsm6ds0_gysensor_mutex);
	...
	else
	{
		...
		if (atomic_read(&obj_i2c_data->suspend) == 0)
		{
			...
			err = LSM6DS0_SetPowerMode(obj_i2c_data->client, enable_status);

			GYRO_LOG("Gysensor not in suspend gysensor_SetPowerMode!, enable_status = %d\n",enable_status);
		}
		else
		{
			GYRO_LOG("Gysensor in suspend and can not enable or disable!enable_status = %d\n",enable_status);
		}
	}
	mutex_unlock(&lsm6ds0_gysensor_mutex);

    if(err != LSM6DS0_SUCCESS)
	{
		printk("LSM6DS0_gysensor_enable_nodata fail!\n");
		return -1;
	}

    printk("LSM6DS0_gysensor_enable_nodata OK!!!\n");
	return 0;
}
/*----------------------------------------------------------------------------*/
static int lsm6ds0_set_delay(u64 ns)
{
	...
	if(value <= 5)
	{
		sample_delay = LSM6DS0_400HZ;
	}
	else if(value <= 10)
	{
		sample_delay = LSM6DS0_200HZ;
	}
	else
	{
		sample_delay = LSM6DS0_100HZ;
	}

	mutex_lock(&lsm6ds0_gysensor_mutex);
	err = LSM6DS0_SetSampleRate(obj_i2c_data->client, sample_delay);
	mutex_unlock(&lsm6ds0_gysensor_mutex);
	if(err != LSM6DS0_SUCCESS ) //0x2C->BW=100Hz
	{
	...
	{	
		...
		obj_i2c_data->fir.sum[LSM6DS0_AXIS_X] = 0;
		obj_i2c_data->fir.sum[LSM6DS0_AXIS_Y] = 0;
		obj_i2c_data->fir.sum[LSM6DS0_AXIS_Z] = 0;
		atomic_set(&obj_i2c_data->filter, 1);
	#endif
	}

    
    GYRO_LOG("gysensor_set_delay (%d)\n",value);
...
static int lsm6ds0_get_data(int* x ,int* y,int* z, int* status)
{
    ...
    //GSE_FUN();
   	mutex_lock(&lsm6ds0_gysensor_mutex);
	LSM6DS0_ReadGyroData(obj_i2c_data->client, buff, 256);
	mutex_unlock(&lsm6ds0_gysensor_mutex);
...
static int LSM6DS0_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int res = 0;
	GYRO_FUN();
	#ifdef LSM6DS0_GYRO_MTK_NEW_SENSOR_ARCH
...
#ifdef LSM6DS0_GYRO_MTK_NEW_SENSOR_ARCH
	if((err = LSM6DS0_create_attr(&lsm6ds0_init_info.platform_diver_addr->driver)))
#else
	if((err) = LSM6DS0_create_attr(&LSM6DS0_gyro_driver.sdriver);
#endif
	{
	...
	ctl.open_report_data= lsm6ds0_open_report_data;	//interrupt mode
	...
	if(err)
	{
		...
		printk("register gyro control path err\n");
		goto exit_create_attr_failed;
	}

	printk("gyro_register_control_path sucess\n");
	...
	if(err)
	{
		GYRO_ERR("register gyro data path err\n");
		printk("register gyro data path err\n");
		goto exit_create_attr_failed;
	}

	printk("gyro_register_data_path sucess\n");
	
	err = batch_register_support_info(ID_GYROSCOPE,ctl.is_support_batch, 1000, 0);
	if(err)
	{
		GYRO_ERR("register gysensor batch support err = %d\n", err);
		printk("register gysensor batch support err\n");
		goto exit_create_attr_failed;
	}
