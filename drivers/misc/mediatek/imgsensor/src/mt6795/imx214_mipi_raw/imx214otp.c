...
{	
	...
	CAM_CALDB("IMX214_Read_module_id  !!\n");
...
static kal_uint8 IMX214_Read_AF_Otp(kal_uint8 address,unsigned char *iBuffer,unsigned int buffersize)
{	
	u8 readbuff, i;

	for(i=0;i<buffersize;i++)
		{
	            iReadCAM_CAL_8((address+i),&readbuff,0xA8);
			*(iBuffer+i)=readbuff;
...
static kal_uint8 IMX214_Read_AWB_Otp(kal_uint8 address,unsigned char *iBuffer,unsigned int buffersize)
{
	u8 readbuff, i;

	for(i=0;i<buffersize;i++)
		{
	            iReadCAM_CAL_8((address+i),&readbuff,0xA0);
			*(iBuffer+i)=readbuff;
 ...
 kal_bool IMX214_Read_LSC_Otp(u16 Outdatalen,unsigned char * pOutputdata)
 {
...
static kal_bool IMX214_Read_LSC_Otp_Ofilm(u16 Outdatalen,unsigned char * pOutputdata)
 ...
 void IMX214_ReadOtp(kal_uint16 address,unsigned char *iBuffer,unsigned int buffersize)
{
		kal_uint16 i = 0;
 ...
 int iReadData(kal_uint16 ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
	...
	else if(ui4_length ==4)
    {
	    ...
	    IMX214_Read_AWB_Otp(ui4_offset, pinputdata, ui4_length);
	   	
    }
	else if(ui4_length ==2)
    {
		
	    IMX214_Read_AF_Otp(ui4_offset, pinputdata, ui4_length);
		...
		{
		    IMX214_Read_LSC_Otp(ui4_length,pinputdata);
		}
